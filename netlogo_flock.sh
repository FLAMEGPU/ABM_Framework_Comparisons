#!/bin/bash

# NetLogo's profiler sucks in the sense that it times one run, then spits out a bunch of junk
# to either a file or stdout. There's no easy abilitiy to parse it.

JAVA_HOME=/usr JAVA_TOOL_OPTIONS=-Xmx8192m NetLogo/netlogo-headless.sh --model "NetLogo/Flocking/Flocking.nlogo" --experiment benchmark | awk '/GO/{i++}i==2{print $3;exit}'
